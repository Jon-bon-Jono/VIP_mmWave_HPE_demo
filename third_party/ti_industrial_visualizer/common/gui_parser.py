...
class UARTParser():
    ...
    def readAndParseUartDoubleCOMPort(self):
        ...
        if (self.parserType == "DoubleCOMPort"):
            if self.device == "xWR6843" and self.demo == DEMO_3D_PEOPLE_TRACKING:
                outputDict = parseCapon3DFrame(frameData)
            else:
    ...
    def connectComPorts(self, cliCom, dataCom):
        ...
        if(self.device == "xWRL6844"):
            ...
            self.dataCom = serial.Serial(dataCom, 1843200, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=0.6)
    ...
    def sendCfg(self, cfg):
        ...
        for line in cfg:
            ...
            if line.strip().startswith("sensorStart"):
                log.info("Waiting before sensorStart to mimic old custom Python startup sequence")
                time.sleep(3.0)
